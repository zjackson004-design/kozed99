...
const ODDS_API_KEY = "30ef25366c0f5e91d4930278ba152e07"; 
...
// ✅ 3. MIDDLEWARE & STATIC FILES (FIXES "Cannot GET /")
app.use(express.static(__dirname));

// Maintenance Check Middleware
app.use(async (req, res, next) => {
    if (req.path.startsWith('/admin') || req.path === '/auth/login' || req.path === '/auth/register' || req.path.includes('.')) {
        return next();
    }
...
// ✅ 5. USER ROUTES (With Negative Balance Fix)
    ...
    try {
        ...
        if (type === 'Withdraw') {
            if (user.balance < amount) {
                return res.status(400).json({ error: "Insufficient Balance (လက်ကျန်ငွေမလောက်ပါ)" });
            }
            user.balance -= amount; 
            await user.save();
        }
...
app.post('/admin/approve-request', async (req, res) => { 
    ...
    try { 
        ...
        if(!reqDoc || reqDoc.status !== 'Pending') return res.status(400).json({ error: "Invalid" }); 
        reqDoc.status = action; 
        await reqDoc.save(); 
        const user = await User.findOne({ username: reqDoc.username }); 
        if(user) { 
            const details = `${reqDoc.method} | ${reqDoc.paymentId} | ${reqDoc.accountName}`; 
            if (action === 'Approve') { 
                if (reqDoc.type === 'Deposit') { 
                    user.balance += reqDoc.amount; 
                    user.transactions.unshift({ title: "Deposit Approved", type: "Deposit", details: details, date: new Date().toLocaleString(), amount: `+${reqDoc.amount}`, status: "Success", color: "green" }); 
                } else { 
                    user.transactions.unshift({ title: "Withdraw Approved", type: "Withdraw", details: details, date: new Date().toLocaleString(), amount: `-${reqDoc.amount}`, status: "Success", color: "red" }); 
                } 
            } else if (action === 'Reject' && reqDoc.type === 'Withdraw') { 
                user.balance += reqDoc.amount; 
                user.transactions.unshift({ title: "Withdraw Refund", type: "Refund", details: "Refund", date: new Date().toLocaleString(), amount: `+${reqDoc.amount}`, status: "Refund", color: "green" }); 
            } 
            await user.save(); 
        } 
        res.json({ success: true }); 
...
// ✅ 7. ODDS API & MOCK DATA (FIXED)
let cachedOdds = []; let lastFetchTime = 0; const CACHE_DURATION = 30 * 60 * 1000; 
...
function getMockMatches() {
    ...
    return [
        { id: 'mock1', home: 'Man United', away: 'Chelsea', league: { name: 'Premier League' }, fixture: { date: today.toISOString() }, ft: { h: '2.10', d: '3.50', a: '2.80', o: '1.90', u: '1.90', line_o: '2.5', hdp_h: '1.95', hdp_a: '1.85', line_h: '-0.5' } },
        { id: 'mock2', home: 'Real Madrid', away: 'Barcelona', league: { name: 'La Liga' }, fixture: { date: today.toISOString() }, ft: { h: '2.50', d: '3.40', a: '2.60', o: '1.85', u: '1.95', line_o: '3.0', hdp_h: '2.00', hdp_a: '1.80', line_h: '0' } }
    ];
}
...
app.get('/odds', async (req, res) => {
    ...
    try {
        ...
        const requests = LEAGUES.map(league => axios.get(`https://api.the-odds-api.com/v4/sports/${league.key}/odds`, { params: { apiKey: ODDS_API_KEY, regions: 'eu', markets: 'h2h,totals,spreads', oddsFormat: 'decimal' } }).then(r => r.data.map(g => { const bookie = g.bookmakers.find(b => b.key === 'bet365') || g.bookmakers[0]; let ft = { h: "-", d: "-", a: "-", o: "-", u: "-", line_o: "2.5", hdp_h: "-", hdp_a: "-", line_h: "0" }; if (bookie) { const h2h = bookie.markets.find(m => m.key === 'h2h'); if (h2h) { ft.h = h2h.outcomes.find(o => o.name === g.home_team)?.price || "-"; ft.a = h2h.outcomes.find(o => o.name === g.away_team)?.price || "-"; ft.d = h2h.outcomes.find(o => o.name === 'Draw')?.price || "-"; } const totals = bookie.markets.find(m => m.key === 'totals'); if (totals) { const over = totals.outcomes.find(o => o.name === 'Over'); const under = totals.outcomes.find(o => o.name === 'Under'); if (over) { ft.o = over.price; ft.line_o = over.point; } if (under) ft.u = under.price; } const spreads = bookie.markets.find(m => m.key === 'spreads'); if (spreads) { const home = spreads.outcomes.find(o => o.name === g.home_team); const away = spreads.outcomes.find(o => o.name === g.away_team); if (home) { ft.hdp_h = home.price; ft.line_h = home.point > 0 ? `+${home.point}` : home.point; } if (away) ft.hdp_a = away.price; } } return { id: g.id, home: g.home_team, away: g.away_team, league: { name: league.name }, fixture: { date: g.commence_time }, ft }; })).catch(e => [])); 
        const results = await Promise.all(requests); 
        let finalData = results.flat().sort((a, b) => new Date(a.fixture.date) - new Date(b.fixture.date)); 
        if (finalData.length === 0) finalData = getMockMatches(); // Use Mock if API empty
        cachedOdds = finalData; lastFetchTime = Date.now(); res.json(finalData); 
    } catch (e) { res.json(getMockMatches()); } // Use Mock if Error
});

// ✅ 8. IMPORTANT: ROUTING (This fixes 'Cannot GET /')
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'login.html')));
app.get('/home', (req, res) => res.sendFile(path.join(__dirname, 'home.html')));
app.get('/admin', (req, res) => res.sendFile(path.join(__dirname, 'admin.html')));
app.get('*', (req, res) => { if(req.accepts('html')) res.sendFile(path.join(__dirname, 'login.html')); else res.status(404).json({error:'404'}); });
